...
function criptografar() {
  ...
  const numEncry = txLength.toString(4).split("").reverse().join("")
  txReordered.shift()
  ...
  for (let i = 0; i < txReordered.length; i++) {
    if (typeof txReordered[i] === "number") {
      ...
      for (let i = 0; i < num.length; i++) {
        let temp = num[i]
        if (temp > 0) {
          let barraP = (temp / 10) | 0
          let setaP = temp % 10
          encrypted += "/".repeat(barraP) + "<".repeat(setaP) + "!"
        } else if (temp < 0) {
          let barraN = (temp / 10) | 0
          let setaN = temp % 10
          encrypted += "\\".repeat(-barraN) + ">".repeat(-setaN) + "!"
        } else {
          encrypted += "!"
        }
      }
  ...
  document.getElementById("saidaText").innerText = encrypted
}